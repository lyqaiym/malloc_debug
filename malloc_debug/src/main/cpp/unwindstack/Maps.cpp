...
bool Maps::Parse() {
  bool p =  android::procinfo::ReadMapFile(
      GetMapsFile(),
  ...
  error_log("Maps::Parse:p=%d,size=%d",p,maps_.size());
  return p;
}
...
bool BufferMaps::Parse() {
  ...
  bool p = android::procinfo::ReadMapFileContent(
      &content[0],
      [&](uint64_t start, uint64_t end, uint16_t flags, uint64_t pgoff, ino_t, const char* name) {
        // Mark a device map in /dev/ and not in /dev/ashmem/ specially.
        if (strncmp(name, "/dev/", 5) == 0 && strncmp(name + 5, "ashmem/", 7) != 0) {
  ...
  error_log("BufferMaps::Parse:p=%d",p);
  return p;
}
...
bool LocalUpdatableMaps::Reparse() {
  ...
  error_log("BufferMaps::Reparse:size1=%d",maps_.size());
  if (!Parse()) {
  ...
  error_log("BufferMaps::Reparse:size2=%d",maps_.size());
  return true;
}
